...
export const payPartnersOptional = asyncHandler(async (req, res, next) => {
  const { amount: amountOptional, program_id } = req.body;
  let program;
  try {
    program = await optionalPrograms.findById(program_id);
  } catch (e) {
    console.log(e);
    return { message: "Опционны не найденны", status: "error" };
  }
  const amount = amountOptional * program.cost;
